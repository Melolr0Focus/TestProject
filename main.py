...
async def lawyer_command(message: Message):
    user_mention = f"@{message.from_user.username}" if message.from_user.username else message.from_user.full_name
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="ТГ", url="")],
        [InlineKeyboardButton(text="Зайти на сервер", url="https://www.roblox.com/games/start?placeId=7711635737&launchData=joinCode=c20e90w3")]
    ])
    await message.answer(f"{user_mention}, Сегодня 05.10 был тяжелый день, наша администрация приняла решение переехать на другой сервер и тг группу. Так же были переработаны все кнопки для нового сервера.\n\n 06.10 Были обновлены некоторые команды, незначительные обновления. \n\n 07.10 Сегодня добавлена эта команда.", reply_markup=keyboard)
